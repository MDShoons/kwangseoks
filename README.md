...
이번 버전은 Firebase를 `type="module"` 방식이 아니라 `compat script` 방식으로 불러옵니다.

이전 문제:
- app.js가 module import에서 멈추면 로그인/회원가입/중복확인이 실행되지 않음
- 로그인 창에 “Firebase 연결 확인 중...”만 계속 남음

v14 수정:
- `index.html`에서 Firebase compat SDK를 먼저 불러옴
- `app.js`는 일반 script로 실행
- import/type=module 오류를 피함
- 로그인/회원가입/중복확인 코드가 더 안정적으로 실행됨

GitHub에 올린 뒤 `https://mdshoons.github.io/kwangseoks/?v=14#home`로 접속하고 Ctrl+F5 하세요.
