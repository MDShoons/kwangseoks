...
## v80 수정 사항 - 구글드라이브 목록 플레이어 UI 정리
구글드라이브 preview iframe이 Songs/Radios 목록 카드 안에서 작게 깨져 보이던 문제를 수정했습니다.

수정:
- 목록 카드에서는 iframe을 직접 표시하지 않음
- 목록에는 흑백 버튼형 “구글드라이브 음원” UI 표시
- 버튼 클릭 시 구글드라이브 preview 새 창 열기
- 상세보기에서는 더 큰 preview iframe 표시
- 상세보기에는 “새 창에서 열기” 버튼과 공유 설정 안내 표시

주의:
Google Drive가 File not found를 표시하면 사이트 문제가 아니라 파일 공유 설정 문제일 가능성이 큽니다.
공유 설정을 “링크가 있는 모든 사용자: 뷰어”로 변경해야 합니다.


## v81 수정 사항 - 직접 mp3 링크 등록 / Songs-Radios 동일 UI
oneum.net 같은 직접 mp3 링크를 URL 방식으로 등록하고, Songs도 Radios와 같은 흑백 커스텀 플레이어로 재생하도록 정리했습니다.

지원:
- http://oneum.net/.../file.mp3
- https://.../file.mp3
- 일반 mp3/wav 직접 파일 링크

중요:
GitHub Pages는 HTTPS 사이트입니다.
브라우저 보안 정책상 HTTPS 페이지에서 HTTP 음원을 직접 재생하면 차단될 수 있습니다.
v81은 http:// 링크를 재생할 때 자동으로 https:// 로 바꿔 시도합니다.
해당 서버가 HTTPS를 지원하지 않으면 브라우저에서 재생이 안 될 수 있습니다.

적용 후 사이트 하단 버전:
v81-direct-mp3-link-player


## v82 수정 사항 - HTTP mp3 링크 재생 지원
GitHub Pages는 HTTPS 사이트라서 http:// 음원을 브라우저가 직접 차단할 수 있습니다.
이를 해결하기 위해 http:// 음원 링크를 Cloudflare Worker 프록시로 재생하도록 변경했습니다.

동작:
- 입력 링크: http://oneum.net/.../song.mp3
- 재생 링크: https://kwangseoks-uploader.kos20050627.workers.dev/proxy?url=...
- 브라우저는 HTTPS 음원처럼 인식
- Range 헤더 전달로 오디오 길이/탐색 지원

중요:
GitHub에 v82를 올리는 것만으로는 부족합니다.
Cloudflare Worker 코드도 ZIP 안의 cloudflare-worker-github-uploader.js로 교체하고 Deploy해야 합니다.
