...
## v19 수정 사항 - 글씨 색상/크기/사이트 색상 설정
관리자 → 템플릿/사이트 수정 화면에 디자인 설정을 추가했습니다.
...
1. 관리자 로그인
2. 관리자 → 템플릿/사이트 수정
3. 글씨/색상 디자인에서 원하는 값 변경
4. 사이트 문구/배경 저장 클릭

색상과 글씨 크기는 Firestore의 siteSettings/main 문서에 저장됩니다.


## v20 수정 사항 - 디자인 설정 즉시 적용 보정
v19에서 색상/글씨 크기를 바꿔도 바로 적용되지 않는 문제를 보완했습니다.

수정 내용:
- 색상/글씨 크기 입력창에 input/change 이벤트를 확실히 연결
- “색상/글씨 크기 바로 미리보기” 버튼 추가
- CSS 변수뿐 아니라 실제 DOM 스타일에도 직접 적용
- 저장 후 Firestore 값이 다시 불러와져도 디자인이 유지되도록 보정
- 기존 CSS가 우선 적용되는 문제를 막기 위해 v20 override CSS 추가

사용 방법:
1. 관리자 → 템플릿/사이트 수정
2. 글씨/색상 디자인 변경
3. 바로 반영되지 않으면 “색상/글씨 크기 바로 미리보기” 클릭
4. 실제 저장은 “사이트 문구/배경 저장” 클릭


## v21 수정 사항 - 카드 클릭 상세 보기
각 페이지의 카드나 목록 항목을 클릭하면 상세 보기 창이 열리도록 수정했습니다.

동작:
- Photos: 사진 크게 보기
- Videos: 유튜브/영상 큰 플레이어로 보기
- Songs/Radios: 오디오 플레이어로 듣기
- Stories/About/Oneum: 글 상세 보기
- 제목, 설명, 연도, 출처, 카테고리 표시
- 닫기 버튼 또는 ESC 키로 닫기


## v22 수정 사항 - 상세 보기 클릭 보정
사진 카드를 눌러도 상세 보기가 열리지 않는 문제를 보완했습니다.

수정 내용:
- index.html에 상세 보기 모달이 없어도 app.js가 자동 생성
- 카드에 data-content-id를 넣어 클릭 인식 안정화
- 카드 전체 클릭 이벤트를 document에서 위임 처리
- 이미지/영상/오디오 내부 클릭과 카드 클릭 충돌 방지
- ESC 키와 닫기 버튼으로 닫기 지원

업로드할 때는 index.html, style.css, app.js를 모두 교체하는 것을 권장합니다.


## v23 수정 사항 - 상단 메뉴 시각 복구
v20~v22에서 디자인 설정 보정 때문에 상단 메뉴가 모두 갈색 버튼처럼 보이는 문제를 수정했습니다.

수정 내용:
- 상단 메뉴는 다시 텍스트형 메뉴로 표시
- 로그인/회원가입/로그아웃 버튼은 버튼형 유지
- 관리자 탭 버튼은 버튼형 유지
- 카드 클릭 상세 보기 기능은 그대로 유지
- 디자인 설정에서 버튼 색을 바꿔도 상단 메뉴는 텍스트형 유지


## v24 안전 복구
v23 적용 후 아무 버튼도 눌리지 않는 문제를 복구하기 위한 버전입니다.

조치 내용:
- app.js 문법 오류 여부를 검사했습니다.
- 오류가 감지되면 안정 버전의 app.js로 되돌렸습니다.
- 상단 메뉴는 텍스트형으로 유지했습니다.
- 상세 보기 모달 CSS는 유지했습니다.
- firebase-config.js는 건드리지 않았습니다.

검사 결과:
- 최종 app.js 문법 검사: 통과

중요:
GitHub에는 index.html, style.css, app.js 세 파일을 같이 올리세요.
firebase-config.js는 덮어쓰지 마세요.
