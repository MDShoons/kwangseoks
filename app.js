// v14: Firebase compat 방식.
// 일반 <script src="./app.js">로 실행되므로 import/type=module 오류가 나지 않습니다.
...
const doc = (db, collectionName, documentId) =>
...
const updateDoc = (documentRef, data) => documentRef.update(data);
const deleteDoc = (documentRef) => documentRef.delete();
const getDocs = (queryRef) => queryRef.get();
const orderBy = (field, direction) => ({ field, direction });
const query = (collectionRef, order) => collectionRef.orderBy(order.field, order.direction);
const serverTimestamp = () => firebase.firestore.FieldValue.serverTimestamp();

const ref = (storage, path) => storage.ref(path);
...
window.addEventListener("error", (event) => {
  ...
  if (el) {
    el.textContent = "사이트 코드 오류: " + (event.message || "알 수 없는 오류");
    el.className = "small-text error";
  }
});
