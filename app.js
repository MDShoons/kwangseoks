...
window.addEventListener("error", (event) => {
  console.error("사이트 스크립트 오류:", event.error || event.message);
  const status = document.getElementById("firebaseStatusText");
  if (status) {
    status.textContent = "스크립트 오류가 있습니다. F12 Console을 확인하세요: " + (event.message || "");
    status.className = "small-text error";
  }
});

window.addEventListener("unhandledrejection", (event) => {
  console.error("사이트 비동기 오류:", event.reason);
  const status = document.getElementById("firebaseStatusText");
  if (status) {
    status.textContent = "Firebase 또는 권한 오류가 있습니다. F12 Console을 확인하세요.";
    status.className = "small-text error";
  }
});
