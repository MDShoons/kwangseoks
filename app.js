...
// v153: 대화 감정/갈등/농담/공감 흐름 보강.
// 152의 인사 흐름 위에 덧씌우는 실제 대화 엔진이다.
// 핵심: 입력을 "주제"로만 보지 않고, 말의 행동(인사/질문/농담/화남/사과/고마움/위로 요청/반박)을 먼저 판정한다.
function telecomCanonicalInput(text) {
  let t = String(text || "").trim();
  const pairs = [
    [/그라고/g, "그리고"], [/글고/g, "그리고"], [/근대/g, "근데"], [/근데/g, "그런데"],
    [/머하/g, "뭐하"], [/모하/g, "뭐하"], [/뭐햐/g, "뭐하"], [/뭐하고잇/g, "뭐하고있"],
    [/잇어/g, "있어"], [/잇냐/g, "있냐"], [/잇나요/g, "있나요"], [/업어/g, "없어"], [/업냐/g, "없냐"], [/업나요/g, "없나요"],
    [/조아/g, "좋아"], [/조은/g, "좋은"], [/됫/g, "됐"], [/되요/g, "돼요"],
    [/얘기/g, "이야기"], [/애기/g, "이야기"], [/예기/g, "이야기"],
    [/광서/g, "광석"], [/광석이형/g, "광석이 형"], [/광석형/g, "광석 형"],
    [/일훈아는/g, "일훈이는"], [/일훈아/g, "일훈아"],
    [/시바/g, "시발"], [/씨바/g, "시발"], [/ㅅㅂ/g, "시발"], [/ㅈㄴ/g, "존나"],
    [/개열받/g, "열받"], [/빡쳐/g, "화나"], [/빡치/g, "화나"],
    [/고맙/g, "고마워"], [/감솨/g, "감사"], [/ㅈㅅ/g, "죄송"], [/미얀/g, "미안"],
    [/농담이야/g, "장난이야"], [/장난임/g, "장난이야"], [/위로좀/g, "위로 좀"], [/공감좀/g, "공감 좀"]
  ];
  pairs.forEach(([re, to]) => { t = t.replace(re, to); });
  return t;
}
function telecomIntent(message) {
  const msg = telecomCompactInput(message);
  const raw = telecomCanonicalInput(message);
  if (/^(y|Y|n|N)$/.test(String(message || "").trim())) return "yn";
  if (/미안|죄송|사과|내가잘못|잘못했/.test(msg)) return "apology";
  if (/고마워|감사|고맙|땡큐|ㄳ/.test(msg)) return "thanks";
  if (/장난|농담|웃기|웃겨|웃김|ㅋㅋ|ㅎㅎ|푸하|하하|키득|개웃/.test(msg)) return "joke";
  if (/시발|씨발|존나|개새|병신|멍청|꺼져|닥쳐|짜증|열받|화나|화났|싸우|싸움|욕|재수없|싫어|싫다/.test(msg)) return "fight";
  if (/아니|아닌데|별로|그건아니|틀렸|말도안|싫은데/.test(msg)) return "disagree";
  if (/맞아|맞습니다|동의|그렇지|그러게|인정/.test(msg)) return "agree";
  if (/뭐하|뭐해|뭐하는|뭐하세요|머하|모하|다들뭐|어디서|지금뭐/.test(msg)) return "doing";
  if (/안녕|하이|어서|반가|왔어요|왔어|접속/.test(msg)) return "greeting";
  if (/비|우울|허전|외롭|힘들|슬프|센치|쓸쓸|답답|피곤|위로|공감|속상|눈물|울고|괴롭|지침/.test(msg)) return "comfort";
  if (/노래|기타|공연|음악|앨범|라디오|음반|자료실/.test(msg)) return "music";
  if (/밥|술|맥주|소주|감자탕|먹|마시/.test(msg)) return "food";
  if (/생일|축하|기념|추카/.test(msg)) return "celebrate";
  if (/왜|어떻게|무슨|궁금|알려|질문|\?/.test(raw)) return "question";
  return "chat";
}
function telecomExtractConcreteThing(text) {
  const raw = telecomCanonicalInput(text).trim();
  if (!raw) return "그 얘기";
  if (/시발|짜증|화나|열받|싸우|욕/.test(raw.replace(/\s+/g,""))) return "화난 얘기";
  if (/미안|죄송|사과/.test(raw)) return "사과 얘기";
  if (/고마|감사/.test(raw)) return "고마운 얘기";
  if (/농담|장난|웃/.test(raw)) return "농담 얘기";
  if (/비|날씨/.test(raw)) return "비 얘기";
  if (/노래|음악|앨범|음반|라디오/.test(raw)) return "노래 얘기";
  if (/기타|코드|스트로크/.test(raw)) return "기타 얘기";
  if (/감자탕|소주|맥주|밥|먹/.test(raw)) return "먹는 얘기";
  if (/자료실|게시판|낙서|원음/.test(raw)) return "자료실 얘기";
  if (/외롭|허전|힘들|답답|우울|위로|공감/.test(raw)) return "마음 얘기";
  const cleaned = raw.replace(/[?？!！.。~]/g, "").slice(0, 18);
  return cleaned ? `${cleaned} 얘기` : "그 얘기";
}
function telecomSetThread(intent, userText = "") {
  const text = String(userText || "");
  ...
  else if (intent === "comfort") topic = "마음 얘기";
  else if (intent === "food") topic = text.includes("감자탕") ? "감자탕 얘기" : "먹는 얘기";
  else if (intent === "celebrate") topic = "축하 얘기";
  else if (intent === "question") topic = "질문 얘기";
  else if (intent === "greeting") topic = "인사";
  else if (intent === "fight") topic = "말다툼";
  else if (intent === "joke") topic = "농담";
  else if (intent === "apology") topic = "사과";
  else if (intent === "thanks") topic = "고마움";
  else if (intent === "agree") topic = "동의";
  else if (intent === "disagree") topic = "반박";
  telecomSaveJson(TELECOM_STORAGE.thread, { intent, topic, userText: text.slice(0, 80), updatedAt: telecomNow() });
  return topic;
}
function telecomMoodForIntent(intent) {
  if (intent === "fight" || intent === "disagree") return "heated";
  if (intent === "comfort") return "soft";
  if (intent === "joke" || intent === "laugh") return "funny";
  if (intent === "apology" || intent === "thanks") return "warm";
  return "normal";
}
function telecomCoolingLine(member) {
  const casual = telecomMemberUsesCasual(member);
  if (member?.nick === "녹차향기") return telecomPickLine(["잠깐만요. 너무 세게 말하진 말아요", "서로 말은 조금만 낮춰요", "싸우지는 말고 천천히 얘기해요"]);
  if (member?.nick === "mouse14") return telecomPickLine(["어어 싸우지마", "후후후.. 말 좀 살살해", "야 그건 좀 세다"]);
  if (member?.nick === "enfant") return telecomPickLine(["음........ 조금 진정해요...", "그렇게까지 말하면 서로 속상해요...", "잠깐 쉬었다 말해도 돼요..."]);
  return casual ? telecomPickLine(["야 말 좀 살살해", "그건 좀 심했다", "싸우진 말자"]) : telecomPickLine(["말은 조금만 낮추시죠", "그건 조금 세게 들려요", "싸우지는 말아요"]);
}
function telecomHumanReactionVerb(intent) {
  const choices = {
    doing: ["보고 있었어요", "정리하고 있었어요", "갈무리하고 있었어요", "잠깐 눈팅중이었어요", "글 읽고 있었어요"],
    music: ["듣고 있었어요", "자료 찾고 있었어요", "곡 얘기 보고 있었어요", "라이브 생각하고 있었어요"],
    comfort: ["조용히 듣고 있었어요", "마음 쓰여서 보고 있었어요", "조금 생각하고 있었어요"],
    fight: ["잠깐 놀랐어요", "말이 좀 세다고 생각했어요", "분위기 보고 있었어요"],
    joke: ["웃고 있었어요", "장난인 줄 알고 봤어요", "분위기 보고 있었어요"],
    apology: ["듣고 있었어요", "괜찮다고 생각했어요"],
    thanks: ["기분 좋게 보고 있었어요"],
...
function telecomContextFollowLine(member, intent, userText = "", previousMember = null) {
  const casual = telecomMemberUsesCasual(member);
  const prevName = previousMember ? (telecomGivenName(previousMember.name) || previousMember.nick) : "";
  if (intent === "greeting") return casual ? telecomPickLine(["반가워", "그래, 어서와", "하이!!!!", "나도 반가워"]) : telecomPickLine(["반갑습니다", "어서오세요", "네, 반갑습니다"]);
  if (intent === "fight") return telecomCoolingLine(member);
  if (intent === "disagree") return casual ? telecomPickLine(["아니 뭐 다르게 볼 수도 있지", "그건 서로 생각이 좀 다른가보다", "말은 되는데 좀 더 들어보자"]) : telecomPickLine(["다르게 볼 수도 있겠네요", "서로 생각이 조금 다른 것 같아요", "조금 더 들어보죠"]);
  if (intent === "joke") return casual ? telecomPickLine(["ㅋㅋㅋ", "장난이었구나", "후후.. 웃기네"]) : telecomPickLine(["하하.. 장난이었군요", "분위기 풀렸네요", "저도 웃겼어요"]);
  if (intent === "apology") return casual ? telecomPickLine(["괜찮아", "그럴 수도 있지", "다시 얘기하면 되지"]) : telecomPickLine(["괜찮습니다", "그럴 수도 있죠", "다시 얘기하면 됩니다"]);
  if (intent === "thanks") return casual ? telecomPickLine(["뭘", "별말씀을", "도움 됐다니 좋네"]) : telecomPickLine(["별말씀을요", "도움 되셨다니 다행이에요", "좋게 봐주셔서 고맙습니다"]);
  ...
  if (intent === "music") return casual ? telecomPickLine(["나도 그 노래 생각난다", "그 얘기면 계속 해봐", "자료실에 있을지도 몰라"]) : telecomPickLine(["저도 그 노래 생각나요", "그 얘기 계속 들어보고 싶네요", "자료실에도 있을 것 같아요"]);
  if (intent === "comfort") return casual ? telecomPickLine(["그 말 들으니까 좀 그렇다. 천천히 얘기해", "괜찮아. 여기서 좀 쉬어", "혼자 붙들고 있진 마"]) : telecomPickLine(["말씀 들으니까 마음이 쓰이네요. 천천히 얘기하세요", "괜찮습니다. 여기서 조금 쉬세요", "혼자 오래 붙들고 계시진 마세요"]);
  if (intent === "food") return casual ? "그 얘기 들으니까 배고프다" : "그 얘기 들으니까 배고파지네요";
  if (intent === "question") return casual ? "나도 그건 궁금해" : "저도 그건 궁금해요";
  return casual ? telecomPickLine(["응", "그렇구나", "그러게", "말 되네"]) : telecomPickLine(["네", "그렇군요", "그러게요", "말씀 이해했어요"]);
}
function telecomHumanMemberLine(member, intent, userText = "", role = "answer") {
  const given = telecomHumanUserDisplayName();
  const honor = telecomMemberAddressUser(member, given);
  ...
  const nick = member?.nick || "";
  const topic = telecomExtractConcreteThing(userText);
  if (role === "cool") return telecomCoolingLine(member);
  if (role === "follow") return telecomContextFollowLine(member, intent, userText, telecomFindMemberByNick(telecomRecentLogItems(6).filter(x => x.kind === "say").slice(-1)[0]?.nick));
  if (role === "askBack") {
    if (intent === "doing") return casual ? `${topicName} 뭐하고 있었어?` : `${topicName} 뭐하고 계셨어요?`;
    if (intent === "music") return casual ? `${topicName} 그 노래 어디서 들었어?` : `${topicName} 그 노래 어디서 들으셨어요?`;
    if (intent === "comfort") return casual ? `${topicName} 오늘 무슨 일 있었어?` : `${topicName} 오늘 무슨 일 있으셨어요?`;
    if (intent === "joke") return casual ? "ㅋㅋ 뭐가 그렇게 웃겨?" : "하하.. 뭐가 그렇게 웃기셨어요?";
    return casual ? `${topicName} 어떻게 생각해?` : `${topicName} 어떻게 생각하세요?`;
  }
  if (intent === "fight") {
    if (nick === "녹차향기") return telecomPickLine([`${honor} 화난 건 알겠는데, 욕부터 하면 대화가 끊겨요`, "잠깐만요. 말은 조금만 낮춰요", "서로 싸우려고 들어온 건 아니잖아요"]);
    if (nick === "mouse14") return telecomPickLine(["야야 왜 그래", "후후후.. 근데 그 말은 좀 세다", "그건 좀 빡칠만 하긴 한데 욕은 줄이자"]);
    if (nick === "enfant") return telecomPickLine(["음........ 그렇게 말하면 서로 기분 상해요...", "조금만 천천히 말해요...", "화난 건 알겠어요..."]);
    if (nick === "ekjw123") return telecomPickLine(["우하하... 근데 싸우진 말자", "말 세다~~~~", "잠깐 진정~~~~"]);
    return casual ? telecomPickLine(["뭐야 왜 싸워", "그건 좀 세게 들린다", "화난 건 알겠는데 말 좀 낮춰"]) : telecomPickLine(["화나신 건 알겠는데 말은 조금만 낮추시죠", "그건 조금 세게 들립니다", "잠깐 진정하고 얘기해요"]);
  }
  if (intent === "disagree") return casual ? telecomPickLine(["아닌 건 아닌 거지", "나도 그건 좀 다르게 봐", "음.. 그 말은 반은 맞고 반은 아닌듯"]) : telecomPickLine(["저는 조금 다르게 봅니다", "그건 다시 생각해볼 수 있겠네요", "말씀은 알겠는데 전부 동의하긴 어려워요"]);
  if (intent === "joke") {
    if (nick === "mouse14") return telecomPickLine(["후후후.. 그건 좀 웃겼다", "푸히히.. 장난이었네", "알가쓰.. 분위기 풀렸어"]);
    if (nick === "ekjw123") return telecomPickLine(["우하하~~~~~~~~~~~~", "ㅋㅋㅋ 웃겨요", "HI~~~~ 장난이었군요"]);
    if (nick === "raincoat") return telecomPickLine(["홍홍.. 웃기네요", "반갑반갑~~~ 분위기 좋아졌네요"]);
    return casual ? telecomPickLine(["ㅋㅋㅋ", "나도 웃겼어", "장난이었구나"]) : telecomPickLine(["하하.. 그러게요", "저도 좀 웃겼어요", "장난이었군요"]);
  }
  if (intent === "apology") {
    if (nick === "녹차향기") return telecomPickLine([`${honor} 괜찮아요. 다시 얘기하면 됩니다`, "괜찮습니다. 말이 오갈 수 있죠", "사과했으면 됐어요. 계속 얘기해요"]);
    return casual ? telecomPickLine(["괜찮아", "그럴 수도 있지", "다시 얘기하면 되지"]) : telecomPickLine(["괜찮습니다", "그럴 수도 있죠", "다시 얘기하면 됩니다"]);
  }
  if (intent === "thanks") return casual ? telecomPickLine(["뭘", "도움 됐다니 좋네", "별말씀을"]) : telecomPickLine(["별말씀을요", "도움 되셨다니 다행이에요", "좋게 봐주셔서 고맙습니다"]);
  if (intent === "agree") return casual ? telecomPickLine(["맞아 그 말이지", "그러니까", "나도 그렇게 봐"]) : telecomPickLine(["맞습니다", "저도 그렇게 봐요", "그 말씀이 맞는 것 같아요"]);
  if (intent === "doing") {
    if (nick === "녹차향기") return telecomPickLine(["저는 게시판 정리하고 있었어요", "광석 아찌 글 올라왔나 보고 있었어요", `${honor} 저는 방금 자료실 확인하고 있었어요`, "방금 들어온 분들 확인하고 있었어요"]);
    if (nick === "mouse14") return telecomPickLine(["후후후.. 난 대화방 보고 있었지", "난 그냥 구경중", "갈무리할 거 있나 보고 있었어", "알가쓰.. 나는 눈팅중"]);
    if (nick === "enfant") return telecomPickLine(["음........ 저는 글 읽고 있었어요", "그냥 조용히 보고 있었어요...", "낙서장 읽고 있었어요..."]);
  ...
  if (intent === "comfort") {
    if (nick === "녹차향기") return telecomPickLine([`${honor} 너무 오래 혼자 생각하지 마세요`, "비 오면 괜히 그럴 때 있죠", "여기서 천천히 얘기하셔도 돼요"]);
    if (nick === "mouse14") return telecomPickLine(["후후후.. 그래도 여기 왔잖아", "그럴 땐 좀 쉬어", "괜히 혼자 그러지마"]);
    return casual ? telecomPickLine(["그랬구나...", "좀 쉬어", "노래 하나 들어", "그런 날 있지"]) : telecomPickLine(["그러셨군요...", "조금 쉬셔도 돼요", "그럴 때 있지요", "좋은 노래 한 곡 들으세요"]);
  }
  if (intent === "food") return casual ? telecomPickLine(["감자탕 좋지", "소주 얘기하니까 배고프네", "난 맥주 생각난다"]) : telecomPickLine(["감자탕 좋지요", "먹는 얘기는 늘 좋네요", "맥주 한 캔 생각나네요"]);
  if (intent === "greeting") return telecomGreetingAnswerLine(member, userText);
  if (intent === "question") return casual ? telecomPickLine(["그건 나도 궁금하네", "누가 알면 말해줘", "음.. 잘 모르겠는데 계속 얘기해봐"]) : telecomPickLine(["그건 저도 궁금해요", "아시는 분 계세요?", "잘은 모르겠지만 계속 말씀해보세요"]);
  return casual ? telecomPickLine(["응, 무슨 말인지 알겠어", "그렇구나", "그러게", "아하... 계속 얘기해봐"]) : telecomPickLine(["네, 무슨 말씀인지 알겠어요", "그렇군요", "그러게요", "계속 말씀해보세요"]);
}
function telecomHumanKksReply(intent, userText = "", target = "user") {
  const s = telecomCurrentSettings();
  const given = telecomHumanUserDisplayName();
  ...
  if (intent === "fight") return telecomPickLine(["어... 욕은 하지 말고요", "잠깐만요. 말은 조금 낮추고", "화난 건 알겠는데 싸우진 맙시다", "그런 말은 서로 아파요"]);
  if (intent === "disagree") return telecomPickLine(["그럴 수도 있죠", "생각이 다를 수 있어요", "천천히 얘기해봐요", "다 맞을 필요는 없죠"]);
  if (intent === "joke") return telecomPickLine(["훗...", "장난이었구나", "웃기네요", "그런 농담 좋아요"]);
  if (intent === "apology") return telecomPickLine(["괜찮아요", "그럴 수도 있죠", "다시 얘기하면 돼요", "마음 풀어요"]);
  if (intent === "thanks") return telecomPickLine(["별말씀을요", "고맙긴요", "좋게 들었다니 다행이네요"]);
  if (intent === "doing") return telecomPickLine(["지금 잠깐 게시판 보고 있어요", "자판 보고 치고 있어요", "자료실 글 좀 보고 있어요", "방금 들어왔어요", "낮에는 좀 바빴어요"]);
  if (intent === "music") return telecomPickLine(["그 노래 좋아요", "라이브로 하면 좀 달라요", "기타로 하면 괜찮아요", "노래는 천천히 해야죠", "자료실에 있나요?"]);
  if (intent === "comfort") return telecomPickLine([`${prefix}그래요`, "그런날 있죠", "괜히 센치해 있지말기", "씩씩하게 살기...", "좀 쉬어요"]);
  if (intent === "food") return telecomPickLine(["감자탕 좋죠", "맥주한캔 생각나네요", "하 좋~~~타", "먹는 얘기 좋네요"]);
...
function telecomScheduleHumanConversationFlow(userText) {
  ...
  if (intent === "greeting") {
    const greeter = telecomFindMemberByNick("녹차향기") || telecomPickMember();
    const second = allTargeted ? telecomPickMember([greeter?.nick]) : null;
    if (greeter) telecomQueue(() => { if (telecomRoomOpen()) telecomSayMember(greeter, telecomGreetingAnswerLine(greeter, fixedText)); }, telecomRand(900, 2000));
    if (second && Math.random() < 0.25) telecomQueue(() => { if (telecomRoomOpen()) telecomSayMember(second, telecomGreetingAnswerLine(second, fixedText)); }, telecomRand(3200, 5600));
    if (telecomKksActive() && Math.random() < 0.35) telecomQueue(() => { if (telecomRoomOpen() && telecomKksActive()) telecomKks(telecomKksGreetingAnswerLine(fixedText)); }, telecomRand(4200, 7200));
    return;
  }

  // 싸움/욕/화남: 한 명은 받아치고, 한 명은 말린다. 무조건 모두가 착하게만 말하지 않게 한다.
  if (intent === "fight" || intent === "disagree") {
    const target = mentionedMember || telecomPickMember();
    const cooler = telecomFindMemberByNick("녹차향기") || telecomPickMember([target?.nick]);
    if (target) telecomQueue(() => { if (telecomRoomOpen()) telecomSayMember(target, telecomHumanMemberLine(target, intent, fixedText, "answer")); }, telecomRand(1000, 2600));
    if (cooler && cooler.nick !== target?.nick) telecomQueue(() => { if (telecomRoomOpen()) telecomSayMember(cooler, telecomHumanMemberLine(cooler, intent, fixedText, "cool")); }, telecomRand(3800, 7200));
    if (telecomKksActive() && Math.random() < 0.55) telecomQueue(() => { if (telecomRoomOpen() && telecomKksActive()) telecomKks(telecomHumanKksReply(intent, fixedText, "user")); }, telecomRand(8200, 13000));
    return;
  }

  // 사과/고마움/농담: 짧게 받아주고, 필요하면 한 명만 더 웃거나 풀어준다.
  if (["apology", "thanks", "joke", "agree"].includes(intent)) {
    const m1 = mentionedMember || telecomPickMember();
    const m2 = telecomPickMember([m1?.nick]);
    if (m1) telecomQueue(() => { if (telecomRoomOpen()) telecomSayMember(m1, telecomHumanMemberLine(m1, intent, fixedText, "answer")); }, telecomRand(900, 2400));
    if (m2 && Math.random() < (intent === "joke" ? 0.6 : 0.35)) telecomQueue(() => { if (telecomRoomOpen()) telecomSayMember(m2, telecomContextFollowLine(m2, intent, fixedText, m1)); }, telecomRand(3600, 7800));
    if (telecomKksActive() && ["apology", "thanks", "joke"].includes(intent) && Math.random() < 0.35) telecomQueue(() => { if (telecomRoomOpen() && telecomKksActive()) telecomKks(telecomHumanKksReply(intent, fixedText, "user")); }, telecomRand(7600, 12000));
    return;
  }

  // 바로 직전 말에 대한 "왜/그래서/맞아/응" 같은 이어받기.
  if (continuation) {
    const m1 = mentionedMember || telecomFindMemberByNick(continuation.lastNick) || telecomPickMember();
    const m2 = telecomPickMember([m1?.nick]);
    if (m1) telecomQueue(() => { if (telecomRoomOpen()) telecomSayMember(m1, telecomHumanMemberLine(m1, intent === "chat" ? (telecomGetThread()?.intent || "chat") : intent, fixedText, "follow")); }, telecomRand(1600, 4200));
    if (m2 && Math.random() < 0.35) telecomQueue(() => { if (telecomRoomOpen()) telecomSayMember(m2, telecomContextFollowLine(m2, telecomGetThread()?.intent || intent, fixedText, m1)); }, telecomRand(7600, 12000));
    return;
  }

  // 직접 지목한 사람/김광석을 부른 경우.
  if (mentionedMember && !kksTargeted && !allTargeted) {
    const replier = telecomPickMember([mentionedMember.nick]);
    telecomQueue(() => { if (telecomRoomOpen()) telecomSayMember(mentionedMember, telecomHumanMemberLine(mentionedMember, intent, fixedText, "answer")); }, telecomRand(1200, 3600));
    if (replier && Math.random() < 0.45) telecomQueue(() => { if (telecomRoomOpen()) telecomSayMember(replier, telecomContextFollowLine(replier, intent, fixedText, mentionedMember)); }, telecomRand(6500, 11500));
    return;
  }
  if (kksTargeted && !allTargeted) {
    if (telecomKksActive()) telecomQueue(() => { if (telecomRoomOpen() && telecomKksActive()) telecomKks(telecomHumanKksReply(intent, fixedText, "user")); }, telecomRand(1600, 4600));
    else {
      const helper = telecomFindMemberByNick("녹차향기") || telecomPickMember();
      if (helper) telecomQueue(() => { if (telecomRoomOpen()) telecomSayMember(helper, "광석 아찌 지금은 안 계신 것 같아요. 글 남겨두면 보실거예요"); }, telecomRand(1800, 4600));
    }
    return;
  }

  // 일반 대화: 답변 1명 + 맞장구 1명. 전체 호출 때만 3명까지.
  const members = telecomSelectFlowMembers(fixedText, allTargeted ? 3 : 2);
  const m1 = members[0] || telecomPickMember();
  if (!m1) return;
  const m2 = members[1] || telecomPickMember([m1.nick]);
  const m3 = members[2] || telecomPickMember([m1.nick, m2?.nick]);
  telecomQueue(() => { if (telecomRoomOpen()) telecomSayMember(m1, telecomHumanMemberLine(m1, intent, fixedText, "answer")); }, telecomRand(1600, 4200));
  if (m2 && Math.random() < 0.58) telecomQueue(() => { if (telecomRoomOpen()) telecomSayMember(m2, telecomContextFollowLine(m2, intent, fixedText, m1)); }, telecomRand(6200, 11000));
  if (allTargeted && m3 && Math.random() < 0.24) telecomQueue(() => { if (telecomRoomOpen()) telecomSayMember(m3, telecomContextFollowLine(m3, intent, fixedText, m2 || m1)); }, telecomRand(11800, 17000));
  if (telecomKksActive() && (kksTargeted || intent === "music" || intent === "comfort") && Math.random() < 0.38) {
    telecomQueue(() => { if (telecomRoomOpen() && telecomKksActive()) telecomKks(telecomHumanKksReply(intent, fixedText, "user")); }, telecomRand(13500, 21000));
  }
}
function telecomPostAmbientConversation() {
  if (!telecomRoomOpen()) return;
  const log = telecomLoadJson(TELECOM_STORAGE.log, []);
  const lastUser = log.slice().reverse().find((x) => x.kind === "say" && x.nick === telecomCurrentSettings().nick);
  if (lastUser && telecomNow() - (lastUser.time || 0) < 45000) return;
  let speaker = null;
  if (Math.random() < 0.10) speaker = telecomMemberPresenceEvent();
  const thread = telecomGetThread();
  const scenarios = thread ? [
    { intent: thread.intent || "chat", seed: thread.userText || thread.topic || "그 얘기" }
  ] : [
  ...
  const m2 = telecomPickMember([m1.nick]);
  telecomQueue(() => { if (telecomRoomOpen()) telecomSayMember(m1, telecomHumanMemberLine(m1, sc.intent, sc.seed, "answer")); }, telecomRand(1200, 3000));
  if (m2 && Math.random() < 0.40) telecomQueue(() => { if (telecomRoomOpen()) telecomSayMember(m2, telecomContextFollowLine(m2, sc.intent, sc.seed, m1)); }, telecomRand(6200, 10800));
}
