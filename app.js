...
try {
  ...
  auth = getAuth(app);
  db = getFirestore(app);
  firebaseReady = Boolean(firebaseConfig.apiKey && !firebaseConfig.apiKey.includes("여기에") && firebaseConfig.projectId);
} catch (error) {
...
function initAuth() {
  ...
  updateFirebaseStatus("Firebase 설정값이 입력되어 있습니다. 가입/로그인이 안 되면 Authentication의 Email/Password 활성화와 Firestore Rules 게시 여부를 확인하세요.", "ok");
