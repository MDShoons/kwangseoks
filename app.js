...
/* --------------------------------------------------------------------------
   광석이네 통신방 - Cloudflare Workers AI 전용 버전
   - 미리 저장된 회원 대사/무료 자동 반응/로컬 템플릿 반응을 전부 제거
   - 사용자가 메시지를 보낼 때만 Cloudflare Worker가 AI 대사를 새로 생성
   - 브라우저는 대화 저장, 화면 표시, Worker 호출만 담당
-------------------------------------------------------------------------- */
const FB_TELECOM_ROOM_ID = "gwangseok-telecom-main-ai-v13-focused-reply";
...
async function fbTelecomEnterRoom(options = {}) {
  ...
  await setDoc(fbTelecomRoomRef(), {
    title: "광석이네 통신방",
    mode: "cloudflare-workers-ai-only",
    updatedAt: serverTimestamp()
  }, { merge: true });
...
async function fbTelecomSendUserMessage(text) {
  ...
  const clean = fbTelecomCleanText(text, "").slice(0, FB_TELECOM_MAX_INPUT);
  if (!clean) return;
