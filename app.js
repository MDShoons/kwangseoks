...
async function uploadFileToGitHubWorker(file, folder = "images") {
  if (!file) return "";
  if (!GITHUB_UPLOAD_WORKER_URL) throw new Error("Cloudflare Worker 업로드 주소가 설정되지 않았습니다. firebase-config.js의 GITHUB_UPLOAD_WORKER_URL을 입력하세요.");
  if (!currentUser) throw new Error("로그인 후 업로드할 수 있습니다.");
  const idToken = await currentUser.getIdToken();
  const formData = new FormData();
  formData.append("file", file);
  formData.append("folder", folder);
  const res = await fetch(GITHUB_UPLOAD_WORKER_URL, { method: "POST", headers: { Authorization: `Bearer ${idToken}` }, body: formData });
  const result = await res.json().catch(() => ({}));
  if (!res.ok) throw new Error(result.error || "GitHub 파일 업로드에 실패했습니다.");
